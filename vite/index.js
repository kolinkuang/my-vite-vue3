...
const resolvePlugin = [
    ...
    ({app, root}) => {
        app.use(async (ctx, next) => {
            ...
            if (!ctx.query.type) {
                ...
                // const {render} = compileTemplate({source: content}); // 将app.vue中的模板 转换成render函数
                const render = compile(content, {mode: 'module'}).code; // 将app.vue中的模板 转换成render函数
                ctx.body = render;
            }
        })
    },
    ...
    ({app}) => {
        app.use(async (ctx, next) => {
            ...
            const content = await getFileContent(process.cwd(), url, true);
...
async function getFileContent(prefix, pathSegments, isToTrim) {
    const filePath = path.resolve(prefix, pathSegments.slice(+!!isToTrim));
    return await fs.readFile(filePath, 'utf-8');
}

async function getFileContentByType(url, type, ctx) {
    const content = await getFileContent(process.cwd(), url, true);
    ctx.type = type;
    ctx.body = content;
}
